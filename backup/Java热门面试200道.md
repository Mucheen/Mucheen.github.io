...
# 94、Redis中跳表的实现原理是什么？
**原理**：扩展的有序链表，通过维护一个多级索引结构实现快速查找。在跳跃表中，每个节点包含一个数据元素和一组指向其他节点的指针。这些指针分布在不同层级，每个层级的指针数量都比下一层少。最底层包含所有元素，最高层包含少数几个元素
**查找性能**：平均O(log N)、最坏O(N)
![Image](https://github.com/user-attachments/assets/6e7b5a47-b00a-4039-ac6a-ae100e4ac554)
[https://cloud.tencent.com/developer/article/2419519](url)

# 95、Redis性能瓶颈时如何处理·？
**内存使用过高**
内存使用过高时，会引发如系统性能下降、内存溢出等问题
优化方法：
定期清理过期数据
数据压缩
分库分表
...
拆分大键
避免存储过大的值
[https://blog.csdn.net/yvchiby/article/details/140874581](url)

# 96、OSI七层模型是什么？
应用层：规定数据的传输协议（HTTP、POP、SMTP等等）
表示层：解决不同系统之间的通信
会话层：会话层建立、管理和终止表示层与实体之间的通信会话
传输层：向高层提供可靠的端到端的网络数据流服务（端口与端口的通信）
网络层：负责在源和终点之间建立连接
数据链路层：规定了0和1的分包形式，确定了网络数据包的形式
物理层：将信息编码成电流脉冲或其它信号用于网上传输
[https://zhuanlan.zhihu.com/p/685931989](url)

# 97、说说AQS吧？
AQS是Java中用于构建锁和其他同步器的基础框架。AQS的核心思想是基于队列的同步器，通过一个先进先出的等待队列来管理线程的排队和唤醒，可以实现各种自定义的同步器，包括互斥锁、信号量、倒计时器等（基于模板方法模式）。
**数据结构**：一个整型的state变量，用于表示同步状态，通过state变量来实现线程的排队和幻想以及锁的获取和释放。
**实现方法**：
独占模式方法：实现独占锁，即占有一个线程能够获取到锁
共享模式方法：实现共享锁，多个线程可以同时获取到锁
[https://blog.csdn.net/qq_67358181/article/details/138729420](url)

# 98、Cookie、Session、Token之间有什么区别？
Token：验证用户身份的令牌，通常用于接口的身份验证，一般获取之后放在请求头中
Session：服务器为了方便存储用户会话信息产生的，客户端访问服务器时，服务器会给每个访问对象创建一个的sessionID存储在服务器，客户端通常会将sessionID存储在cookie中，在发送请求时一并发给服务器
Cookie：服务器发送到客户端然后存储在客户端的，可以用来保持用户的登录信息

Cookie跟Session的区别：
存储位置不同：cookie存放在客户端，session存放在服务端
大小限制不同：cookie存储在客户端，数据大小是有限制的，session没有大小限制
安全性不同：cookie的数据在客户端容易被篡改
时效性不同：cookie的时效性短，在服务器设定的时间后失效，session时效性由服务器控制，一般在关闭浏览器断开服务器连接时失效
[https://www.cnblogs.com/tyl-bok/p/18200048](url)

# 99、什么是分库分表？分库分表有哪些类型（或策略）？
**概念**：分库分表是数据库架构设计策略，用于处理数据量过大，单一数据库无法承载的情况。通过将数据分散到多个数据库或多个数据表中，分库分表能有效提高数据库的扩展性、性能和可维护性。主要目的是提升查询效率，减轻单个数据库的负载，确保系统的高可用性和高性能。
**类型**：
水平分库分表：数据按照某种规则分散到多个数据库或多个表中
垂直分库分表：数据表中的列拆分到多个表中，每个表存储部分字段，常常用于表的列数过多或某些列的访问频率较低
混合分库分表：
范围分库分表
哈希分库分表
[https://www.cnblogs.com/eiffelzero/p/18607999](url)

# 100、MyBatis中#{}和${}的区别是什么？
#{}是占位符，在预编译时执行
${}是拼接符，能在编译时执行

#{}能防止sql注入，${}不能防止sql注入：由于预编译机制。预编译完成后，sql的结构已经固定，即使用户输入非法参数，也不会对SQL结构产生影响，避免潜在风险。

**总结**：
能用#{}的地方就用#{}；
表名做参数，或者order by排序时用${}；
传参时参数使用@Param("")注解，@Param注解的作用是给参数命名，参数命名后就能根据名字得到参数值（相当于又加了一层密）。
[https://blog.csdn.net/X_islu/article/details/134689105](url)